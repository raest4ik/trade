...
`production-dry-run-burnin-policy-v1` is versioned in source code. Environment variables
cannot weaken its thresholds:

```text
MIN_DISTINCT_MOEX_TRADING_DAYS=5
MIN_PRIMARY_CYCLES=5
...
```powershell
uv run python -m apps.cli.production_dry_run_burnin run-once
```

The runner verifies a real TQBR session from the free official MOEX ISS daily-candle
endpoint. Missing or malformed evidence produces `MOEX_SESSION_STATUS_UNKNOWN` and no
...
```powershell
uv run python -m apps.cli.production_dry_run_burnin status
uv run python -m apps.cli.production_dry_run_burnin report
uv run python -m apps.cli.production_dry_run_burnin inspect <observation_id>
uv run python -m apps.cli.production_dry_run_burnin verify
```
...
Collection remains `IN_PROGRESS` and readiness remains `NO` until at least five confirmed
independent MOEX trading days have been observed. Even a later `YES` never enables paper
execution or scheduling; promotion requires a separate human-reviewed change.
