...
from src.market_data.application.exceptions import MarketDataPartialProviderError
...
async def test_reaction_uses_candle_start_equal_to_published_at_as_effective_event(
    tmp_path: Path,
) -> None:
    session_factory = await _session_factory(tmp_path)
    async with session_factory() as session:
        ...
        news = (
            await news_repository.save(
                NewsItem.create(
                    source_id="source-equal",
                    source_name="Source",
                    source_url="https://example.com/equal",
                    title="SBER",
                    raw_content="SBER news",
                    language="en",
        ...
        await market_repository.save_candles(
            [
                _candle(instrument.id, datetime(2026, 7, 1, 6, 59, tzinfo=UTC), "100"),
                _candle(instrument.id, datetime(2026, 7, 1, 7, 0, tzinfo=UTC), "100"),
                _candle(instrument.id, datetime(2026, 7, 1, 7, 1, tzinfo=UTC), "101"),
            ]
        )
    ...
    reaction = result.reactions[0]
    assert reaction.effective_event_at == datetime(2026, 7, 1, 7, 0, tzinfo=UTC)
    assert reaction.publication_to_effective_event_ms == 0
    assert reaction.points[0].simple_return == Decimal("0.01")


async def test_backfill_finishes_partial_and_counts_existing_rows_on_provider_partial_failure(
    tmp_path: Path,
) -> None:
    session_factory = await _session_factory(tmp_path)
    async with session_factory() as session:
        ...
        candle = _candle(instrument.id, datetime(2026, 7, 1, 7, 0, tzinfo=UTC), "100")
        await market_repository.save_candles([candle])
        result = await BackfillInstrumentCandles(
            instrument_repository=instrument_repository,
            market_data_repository=market_repository,
            provider=_PartialProvider(candle),
        ).execute(
            BackfillInstrumentCandlesCommand(
                instrument_id=instrument.id,
                date_from=datetime(2026, 7, 1, tzinfo=UTC).date(),
                date_till=datetime(2026, 7, 1, tzinfo=UTC).date(),
            )
        )

    assert result.import_record.status.value == "PARTIAL"
    assert result.import_record.error_code == "PROVIDER_PARTIAL"
    assert result.import_record.rows_inserted == 0
    assert result.import_record.rows_existing == 1
    assert result.import_record.rows_valid == 1
...
class _PartialProvider:
    def __init__(self, candle: MarketCandle) -> None:
        self._candle = candle

    async def fetch_candles(self, **_kwargs: object):
